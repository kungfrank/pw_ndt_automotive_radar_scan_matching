    ...
    class NormalDist
    {
      ...
      public:
        ...
        test (const PointT& transformed_pt, const double& cos_theta, const double& sin_theta) const
        {
          ...
          double n_weight = transformed_pt.intensity; // I ---------------------------------- Weighted by intensity
...
pcl::WeightedNormalDistributionsTransform2D<PointSource, PointTarget>::computeTransformation (PointCloudSource &output, const Eigen::Matrix4f &guess)
{
  ...
  while (!converged_)
  {
    ...
    if (nr_iterations_ > max_iterations_ ||
      ...
      std::cout << "iterations_: " << nr_iterations_  << " final eps_: " << (transformation - previous_transformation_).array ().abs ().sum () << std::endl;
      converged_ = true;
    }
