...
Eigen::Matrix4f ndt2d(pcl::PointCloud<pcl::PointXYZ>::Ptr old_in_pc, pcl::PointCloud<pcl::PointXYZ>::Ptr in_pc,
  ...
  pcl::NormalDistributionsTransform2D<pcl::PointXYZ, pcl::PointXYZ> ndt_2d;

  ndt_2d.setMaximumIterations (ndt2d_iteration);
  ndt_2d.setTransformationEpsilon (ndt2d_transformation_epsilon);
  ...
  ndt_2d.setInputSource (old_in_pc);
  ndt_2d.setInputTarget (in_pc);
  pcl::PointCloud<pcl::PointXYZ>::Ptr output_cloud (new pcl::PointCloud<pcl::PointXYZ>);
  ndt_2d.align (*output_cloud, init_guess.inverse());
  //ndt_2d.align (*output_cloud);
  std::cout << "2D NDT has converged: " << ndt_2d.hasConverged ();

  Eigen::Matrix4f Tm;
  Tm = ndt_2d.getFinalTransformation();

  if( !ndt_2d.hasConverged() || isNanInMatrix(Tm) ){
    cout << " NDT2D Fail !! try increasing size... \n";
    Tm = ndt2d(old_in_pc, in_pc, init_guess, grid_step_float + 5.0); // increasing grid size
  }
  else{
    //cout << " grid_step_float: " << grid_step_float << " ";
    //cout << "isNanInMatrix: " << isNanInMatrix(Tm) << " ";
    cout << " score: " << ndt_2d.getFitnessScore (0.5) << std::endl;
    ...
    if(viz){
      ...
      viewer_final->addPointCloud<pcl::PointXYZ> (in_pc, target_color, "target cloud");
      ...
      viewer_final->addPointCloud<pcl::PointXYZ> (old_in_pc, input_color, "input cloud");
      viewer_final->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE,
                                                      5, "input cloud");
      // Starting visualizer
...
void Callback(const sensor_msgs::PointCloud2ConstPtr& input, const nav_msgs::OdometryConstPtr& ego_motion){
  ...
  pcl::fromPCLPointCloud2(*in_pc2, *in_pc);
    ...
    /*
		...
		ndt.setMaximumIterations (ndt_iteration);
		// Setting point cloud to be aligned.
		ndt.setInputSource (old_in_pc);
		// Setting point cloud to be aligned to.
		ndt.setInputTarget (in_pc);
		// Set initial alignment estimate found using robot odometry.
		Eigen::Matrix4f init_guess = getInitGuess(ego_motion, old_ego_motion);
		// Calculating required rigid transform to align the input cloud to the target cloud.
		pcl::PointCloud<pcl::PointXYZ>::Ptr output_cloud (new pcl::PointCloud<pcl::PointXYZ>);
		ndt.align (*output_cloud, init_guess.inverse());
		init_guess = getTransform(0,0,0);
		//std::cout << "Normal Distributions Transform has converged:" << ndt.hasConverged ()
		//          << " score: " << ndt.getFitnessScore () << std::endl;
		Eigen::Matrix4f Tm = ndt.getFinalTransformation();
    */

    Eigen::Matrix4f init_guess = getInitGuess(ego_motion, old_ego_motion);
    Eigen::Matrix4f Tm = ndt2d(old_in_pc, in_pc, init_guess, ndt2d_grid_step);
...
int main (int argc, char** argv)
{
  ...
  ros::NodeHandle nh("~");
  /*
  if (nh.getParam("ndt_transformation_epsilon", ndt_transformation_epsilon)) ROS_INFO("Got 'ndt_transformation_epsilon' param: %.2f", ndt_transformation_epsilon);
  else ROS_WARN("[%s] Failed to get param 'ndt_transformation_epsilon', use default setting: %.2f", ros::this_node::getName().c_str(), ndt_transformation_epsilon);

  if (nh.getParam("ndt_step_size", ndt_step_size)) ROS_INFO("Got 'ndt_step_size' param: %.2f", ndt_step_size);
  else ROS_WARN("[%s] Failed to get param 'ndt_step_size', use default setting: %.2f ", ros::this_node::getName().c_str(), ndt_step_size);

  if (nh.getParam("ndt_resolution", ndt_resolution)) ROS_INFO("Got 'ndt_resolution' param: %.2f", ndt_resolution);
  else ROS_WARN("[%s] Failed to get param 'ndt_resolution', use default setting: %.2f ", ros::this_node::getName().c_str(), ndt_resolution);

  if (nh.getParam("ndt_iteration", ndt_iteration)) ROS_INFO("Got 'ndt_iteration' param: %d", ndt_iteration);
  else ROS_WARN("[%s] Failed to get param 'ndt_iteration', use default setting: %d ", ros::this_node::getName().c_str(), ndt_iteration);
  */
  ...
  message_filters::Subscriber<sensor_msgs::PointCloud2> sub_stack_pc(nh, "/input_pc", 2000);
  ...
  pub = nh.advertise<nav_msgs::Odometry> ("ndt_odom", 200);
