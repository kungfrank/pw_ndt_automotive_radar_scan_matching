...
void radar_sensor(const conti_radar::MeasurementConstPtr& input, float beta, float r_x, float r_y, Eigen::MatrixXd& M_mat , Eigen::MatrixXd& R_mat, Eigen::MatrixXd& VD_mat){
	double valid_size = 0;
	for(int i=0;i<(input->points.size());i++)if ((input->points[i].invalid_state) == 0x00 ||
 (input->points[i].invalid_state) == 0x04 ||
 (input->points[i].invalid_state) == 0x08 ||
 (input->points[i].invalid_state) == 0x09 ||
 (input->points[i].invalid_state) == 0x0a ||
 (input->points[i].invalid_state) == 0x0b ||
 (input->points[i].invalid_state) == 0x0c ||
 (input->points[i].invalid_state) == 0x0f ||
 (input->points[i].invalid_state) == 0x10 ||
 (input->points[i].invalid_state) == 0x11
 ) {valid_size++;}
...
int main (int argc, char** argv)
{
  ...
  pub = nh.advertise<nav_msgs::Odometry> ("vel_valid3", 1);
