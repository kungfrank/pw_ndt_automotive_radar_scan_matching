...
Eigen::Matrix3d getVariance(float td, const nav_msgs::Odometry& vel){
  ...
  return cov_mat * static_cast<double> (td);
}

Eigen::Matrix3d getFixedVariance(float td, const nav_msgs::Odometry& vel){
  Eigen::Matrix3d cov_mat;
  ///------------------- fix uncertainty !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! -------------------///
  cov_mat(0,0) = 0.002;  cov_mat(0,1) = 0;  cov_mat(0,2) = 0;
  cov_mat(1,0) = 0;  cov_mat(1,1) = 0.002;  cov_mat(1,2) = 0;
  cov_mat(2,0) = 0;  cov_mat(2,1) = 0;  cov_mat(2,2) = 6.4e-6; //6.4e-6
  return cov_mat;
}
...
vector<Eigen::Matrix2d> PropagateCov(sensor_msgs::PointCloud2::Ptr pc_msg,
  ...
  for(int i=0; i<pc_new->width; i++){ new_vec_pc_cov.push_back(Eigen::Matrix2d::Zero());}
  return new_vec_pc_cov;
}
...
struct PointCloudWithCov RecursiveStackFrame(vector<nav_msgs::Odometry::ConstPtr> vec_odom_msg,
  ...
  if(vec_odom_msg.size() == stack_frame_num){
    ...
    for(int i=0; i<vec_pc2_msg[0]->width; i++){ new_vec_pc_cov.push_back(Eigen::Matrix2d::Zero());}
  }
  else{
    ...
    //Eigen::Matrix3d var_mat_ego = getFixedVariance(curr_t-old_t, *vec_odom_msg[0]);
    Eigen::Matrix3d var_mat_ego = getVariance(curr_t-old_t, *vec_odom_msg[0]);
    //cout << "var_ego: \n" << var_ego <<endl;
                    ...
                    const nav_msgs::OdometryConstPtr& vel) 
{
  ...
  invalid_pub.publish(*invalid_pc);
    ...
    tb.sendTransform(tf::StampedTransform(transform_map_to_origin, ros::Time::now(), "/map", "/ro_origin"));
...
int main (int argc, char** argv)
{
  ...
  stack_pub = nh.advertise<sensor_msgs::PointCloud2> ("radar_stack", 100);
  cov_pub = nh.advertise<ro_msg::Cov2DArrayStamped> ("radar_stack_cov", 100);

  index_array_pub = nh.advertise<ro_msg::IntArrayStamped> ("/radar_stack_index", 100);
