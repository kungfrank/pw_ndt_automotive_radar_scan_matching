...
int main (int argc, char** argv)
{
  ...
  ros::init (argc, argv, "downsampled_semi_dense");
  ros::NodeHandle nh("~");

  message_filters::Subscriber<sensor_msgs::PointCloud2> sub_stack_pc(nh, "/conti_stack_cov/radar_stack", 1000);
  message_filters::Subscriber<ro_msg::IntArrayStamped> sub_stack_index(nh, "/conti_stack_cov/radar_stack_index", 1000);
  message_filters::Subscriber<ro_msg::Cov2DArrayStamped> sub_cov(nh, "/conti_stack_cov/radar_stack_cov", 1000);
  ...
  pub_stack_pc = nh.advertise<sensor_msgs::PointCloud2> ("radar_stack", 10);
