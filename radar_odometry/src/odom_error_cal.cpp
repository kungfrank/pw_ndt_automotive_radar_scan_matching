/********************************************************
This is the node for odometry error caculation, cacualte both interval and accumulate error.
-------------------------------
INPUT Topic:
  Ground Truth Odometry: (nav_msgs::Odometry)
		/gt_odom
  Target Odometry: (nav_msgs::Odometry)
		/radar_ego_motion
-------------------------------
OUTPUT Topic:
	Odomtery Error: (nav_msgs::Odometry)
		accumulate_odom_error
		interval_odom_error

P.S.
	(OUTPUT).pose.pose.position.z --> Translation error
	(OUTPUT).pose.pose.orientation.z --> Rotation error
