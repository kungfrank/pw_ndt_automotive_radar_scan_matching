...
void error_cb(nav_msgs::Odometry error){
	if(init == false){init = true;} // Skip fisrt error msg
	else{
		n++;
    ...
    old_theta_error_sq = old_theta_error_sq + pow(theta_error/180*M_PI,2); // in radian
		...
		cout << ros::this_node::getName() << " trans error mean: " << (old_trans_error/n) << endl;
		cout << ros::this_node::getName() << " theta error mean: " << (old_theta_error/n) << endl;
